...
class SessionHandler:
    ...
    def _build_session_data(self, session: 'SessionContext') -> Dict[str, Any]:
        ...
        cached_rounds = session.rounds[-self.max_cached_rounds:] if session.rounds else []
    ...
    def _load_session_from_file(self, channel: str, session_id: str) -> List[Dict[str, Any]]:
        """
    ...
    def _append_to_session_file(self, channel: str, session_id: str, role: str, content: str):
        """
        追加一条记录到 session 文件
        ...
        try:
            ...
            logger.debug(f"Appended to {file_path}: {role}")
        except Exception as e:
            logger.warning(f"Failed to append to {file_path}: {e}")
    
    def save_session_to_file(self, channel: str, session_id: str, role: str, content: str):
        """
        ...
        Args:
            ...
            content: 消息内容
        """
        self._append_to_session_file(channel, session_id, role, content)
    
    def get_session_history_from_file(self, channel: str, session_id: str = None, limit: int = 10) -> List[Dict[str, Any]]:
        """
        从文件获取 session 历史记录
