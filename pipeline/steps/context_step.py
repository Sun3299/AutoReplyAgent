...
class ContextStep(Step):
    ...
    def _do_execute(self, ctx: 'PipelineContext') -> StepResult:
        ...
        try:
            ...
            self.session_manager.save_to_file(channel, session_id, "user", user_message)
            self.session_manager.save_to_file(channel, session_id, "assistant", assistant_message)
            ...
            if intent:
                ...
                if state_updates:
                    self.session_manager.update_state(prefixed_session_id, state_updates)
            ...
            return StepResult(
                ...
                data={
                    "session_id": prefixed_session_id,
                    "truncate_marker": truncate_marker.to_dict() if truncate_marker else None,
                },
                step_name=self.name,
                step_type=self.step_type.value,
                duration=duration,
    ...
    def _extract_state_updates(self, intent: Any) -> Dict[str, str]:
        ...
        try:
            if hasattr(intent, "intent_type"):
                state["intent_type"] = intent.intent_type.value if hasattr(intent.intent_type, "value") else str(intent.intent_type)
            
            if hasattr(intent, "query_type") and intent.query_type:
                state["query_type"] = intent.query_type.value if hasattr(intent.query_type, "value") else str(intent.query_type)
            
            if hasattr(intent, "action_type") and intent.action_type:
                state["action_type"] = intent.action_type.value if hasattr(intent.action_type, "value") else str(intent.action_type)
