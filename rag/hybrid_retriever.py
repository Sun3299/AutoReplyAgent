...
class HybridRetriever:
    ...
    def __init__(
        ...
        self.vector_store = ChromaVectorStore(
            ...
            embedding_dim=self.embedding_fn.dim
        )
    ...
    def _rrf_fusion(
        ...
        # 构建结果列表
        results = []
        for rank, (content, rrf_score) in enumerate(sorted_contents[:self.fusion_top_k * 2]):
            # 查找来源
            source = "unknown"
...
def get_retriever(
    ...
    Returns:
        HybridRetriever 实例
    """
    from config.channel_manager import get_channel_config, load_knowledge, get_vector_store_path
...
def _init_default_knowledge(retriever: HybridRetriever):
    ...
    default_docs = [
        ("我们的退货政策是7天内无理由退货，需保持商品完好。退货时请联系客服申请。", "退货政策"),
        ("订单发货后1-3天可查看物流信息，请点击订单详情查看物流动态。", "物流查询"),
        ("商品价格以页面显示为准，下单时可享受当前优惠活动，更多折扣请关注我们的促销页面。", "价格说明"),
        ("会员积分可用于抵扣订单金额，1积分=1分钱，积分越多抵扣越多。", "会员积分"),
        ("客服工作时间为工作日9:00-18:00，非工作时间请留言，我们会尽快回复。", "客服时间"),
