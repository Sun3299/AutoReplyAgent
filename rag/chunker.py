"""
语义切块器 - 已废弃，请使用 advanced_chunker.py 的 HierarchicalChunker
...
class SemanticChunker:
    ...
    def __init__(
        self, min_chars: int = 100, max_chars: int = 500, overlap_chars: int = 50
    ):
        """
        初始化语义切块器
    ...
    def chunk(self, text: str, metadata: Optional[Dict] = None) -> List[Dict]:
        ...
        if current_chunk and len(current_chunk) >= self.min_chars:
            ...
            chunks[-1]["metadata"]["end"] = chunks[-1]["metadata"]["start"] + len(
                chunks[-1]["content"]
            )
        elif current_chunk:
            # 唯一的一块，直接添加
    ...
    def _split_long_sentence(
        self, sentence: str, start_pos: int, metadata: Optional[Dict]
    ) -> List[Dict]:
        """
        分割过长的句子
    ...
    def _create_chunk(self, content: str, start: int, metadata: Optional[Dict]) -> Dict:
        """
        创建块对象
